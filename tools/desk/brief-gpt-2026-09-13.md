...
| Q4 | 오늘 항목 상태 (2026-09-14 새벽 개정·구현) | 왼쪽 오늘 목록 **줄마다 [완료 \| 진행 중 \| 내일로]** 세그먼트(체크박스 대체, 누르면 즉시 todo.md 저장). 진행 중 → "어디까지" 한 줄(선택). 내일로 = 열림 그대로(진행 메모는 메모 줄로 보존) → 내일 아침 어제 이어가기, 2회 연속이면 쪼개기 경고 자리. **기본값 = 현재 상태**(열림→내일로, 진행 중→진행 중), 완료만 직접 — 손 안 댄 항목이 진행 중으로 남지 않는다. 보류·취소는 ⋯ 메뉴(M00-09). 퇴근 폼은 같은 줄을 다시 보여 주고 확인만(M00-03). 한 버튼 순환식은 채택 안 함(현재 상태가 안 보이고 지나치기 쉬움). 옛 결정 "[완료\|진행 중\|보류] 기본 진행 중, 메모 전부 선택"은 폐기 |
